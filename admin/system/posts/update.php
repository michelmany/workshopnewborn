...
<div class="page-title">
	...
	<div class="breadcrumb-env">
		<ol class="breadcrumb bc-1" >
			...
			<li><a href="painel.php?exe=albuns/index">Posts</a></li>
			<li class="active"><strong>Editar post</strong></li>
		</ol>
	</div>
				...
				<div class="panel-body">
			        ...
			        if (isset($post) && $post['SendPostForm']):
		                ...
		                if (!empty($_FILES['gallery_covers']['tmp_name'])):
		                    ...
		                    $sendGallery->gbSend($_FILES['gallery_covers'], $albumid, $userlogin['user_id']);
		                endif;
			        endif;
			        ?>		
		        	...
		        	# Faz A leitura do Banco e mostra os dados na tela
	           		$read = new Read;
	           		$read->ExeRead("nit_posts", "WHERE post_id = :id AND user_id = :user", "id={$albumid}&user={$userlogin['user_id']}");
					...
					<form name="PostForm" action="" method="post" class="form-horizontal" enctype="multipart/form-data">
							...
							<div class="col-sm-9"> 
								<div class="fileinput fileinput-exists" data-provides="fileinput">
								  ...
								  <div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 360px; max-height: 240px;">
								  	<img src="../uploads/<?php if(isset($post['post_capa'])) echo $post['post_capa']; ?>?<?= time(); ?>"> <!-- O time é para limpar o cache -->
								  </div>
								  <div class="text-center">
								    ...
								    <input type="file" name="post_capa"></span>
						...
						<div class="form-group"> 
							...
							<div class="col-sm-9"> 
								<input type="text" class="form-control" id="field-1" placeholder="Ex.: Maria e João" name="post_nome" value="<?php if (isset($post['post_nome'])) echo ($post['post_nome']); ?>"> 
							</div> 
						</div>
						...
						<div class="form-group"> 
							...
							<div class="input-group">
								...
								<input type="text" class="form-control" id="field-1" placeholder="81492863" name="post_video" value="<?php if (isset($post['post_video'])) echo ($post['post_video']); ?>"> 
							</div> 
							</div>
						</div>	
						...
						<div class="form-group"> 
							...
							<div class="col-sm-9"> 
								<input type="text" class="form-control" id="field-1" placeholder="Ex.: Niterói / RJ" name="post_local" value="<?php if (isset($post['post_local'])) echo ($post['post_local']); ?>"> 
							</div> 
						</div>	
						...
						<div class="form-group"> 
							...
							<div class="col-sm-9"> 
								<select class="form-control" id="categoriaSelect" name="post_categoria_id">
								<?php
								$read = new Read;
								$read->ExeRead('nit_posts_cats', "WHERE user_id = :id", "id={$userlogin['user_id']}");
								?>

								<?php if($read->getResult()): ?>
									<?php foreach ($read->getResult() as $categoria): ?>						
									<?php extract($categoria); ?>
										<option <?php if (isset($post['post_categoria_id']) && $post['post_categoria_id'] == $category_id) echo 'selected'; ?> value="<?php echo $category_id; ?>"><?php echo $category_title; ?></option>
									<?php endforeach; ?>
							  	<?php endif; ?>
						...
						<div class="form-group"> 
							...
							<div class="col-sm-9"> 
								<div class="input-group"> 
									...
									<input type="text" class="form-control datepicker" data-format="dd/mm/yyyy" name="post_data" value="<?php if (isset($post['post_data'])) echo date('d/m/Y', strtotime($post['post_data'])); ?>"> 
								</div>
							</div> 
						</div>
			            ...
			            <div class="row thumbsAlbumEdit">
			            	...
			            	<div class="col-sm-9">
								...
								<?php  
								$Gallery = new Read;
								$Gallery->ExeRead("nit_posts_imgs", "WHERE post_id = :album ORDER BY gallery_order ASC", "album={$albumid}");
								...
								if($Gallery->getResult()):
									...
									<div class="view view-first">
										...
										<div class="mask">
											...
											<a onclick="confirm_modal('painel.php?exe=posts/update&albumid=<?= $albumid; ?>&gbdel=<?= $gallery_id; ?>')" class="btn btn-danger">
						...
						<div class="row col-lg-12 col-md-12 text-center">				
							<input type="submit" class="btn btn-success btn-lg" value="Atualizar Álbum" name="SendPostForm">
