...
/**
 * AdminPostCategory.class [ MODEL ADMIN ]
 * Responsável por gerenciar as categorias do Blog do sistema no admin!
 * 
 * @copyright (c) 2015, Michel Moraes NITDESIGN
 */
class AdminPostCategory {
    ...
    public function ExeDelete($CategoryId, $UserID) {
        ...
        if (!$read->getResult()):
            ...
            if (!$this->checkPosts()):
                ...
                header('Location: painel.php?exe=posts/categorias');                
            endif;
        endif;
    }
    ...
    private function checkPosts() {
        $readPosts = new Read;
        $readPosts->ExeRead("nit_posts", "WHERE post_categoria_id = :category", "category={$this->CatId}");
