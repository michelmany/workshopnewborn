...
class AdminPost {
    ...
    public function ExeCreate(array $Data) {
        ...
        $this->setName();
            ...
            if ($this->Data['post_capa']):
                $uplaod = new Upload;
                $uplaod->Image($this->Data['post_capa'], 'postcapa-'.$this->Data['post_url'], null, 'images/posts');
            endif;

            if (isset($uplaod) && $uplaod->getResult()):
                $this->Data['post_capa'] = $uplaod->getResult();
                $this->Create();
            else:
                $this->Data['post_capa'] = null;
    ...
    public function ExeUpdate($PostId, array $Data) {
        ...
        $this->UserID = (int) $Data['user_id'];     
            ...
            if (is_array($this->Data['post_capa'])):
                $readCapa = new Read;
                $readCapa->ExeRead(self::Entity, "WHERE post_id = :post", "post={$this->Post}");
                $capa = '../uploads/' . $readCapa->getResult()[0]['post_capa'];
                if (file_exists($capa) && !is_dir($capa)):
                    unlink($capa);
                endif;

                $uploadCapa = new Upload;
                $uploadCapa->Image($this->Data['post_capa'], 'postcapa-'.$this->Data['post_url'], null, 'images/posts');
            endif;

            if (isset($uploadCapa) && $uploadCapa->getResult()):
                $this->Data['post_capa'] = $uploadCapa->getResult();
                $this->Update();
            else:
                unset($this->Data['post_capa']);
    ...
    public function ExeDelete($PostId, $UserID) {
        ...
        if (!$ReadPost->getResult()):
            ...
            if (file_exists('../uploads/' . $PostDelete['post_capa']) && !is_dir('../uploads/' . $PostDelete['post_capa'])):
                unlink('../uploads/' . $PostDelete['post_capa']);
            endif;

            $readGallery = new Read;
            $readGallery->ExeRead("nit_posts_imgs", "WHERE post_id = :id", "id={$this->Post}");
            ...
            $deleta = new Delete;
            $deleta->ExeDelete("nit_posts_imgs", "WHERE post_id = :gbpost", "gbpost={$this->Post}");
            $deleta->ExeDelete(self::Entity, "WHERE post_id = :postid", "postid={$this->Post}");

            $this->Error = ["O Álbum <b>{$PostDelete['post_nome']}</b> foi removido com sucesso!", WS_ACCEPT];
            $this->Result = true;
     ...
     */
    public function gbSend(array $Images, $PostId, $UserId) {
        $this->Post = (int) $PostId;
        $this->Data = $Images;
        ...
        if (!$ImageName->getResult()):
            ...
            $ImageName = $ImageName->getResult()[0]['post_url'];
            ...
            foreach ($gbFiles as $gbUpload):
                ...
                $gbSend->Image($gbUpload, $ImgName, 1240, 'images/user-id'.$this->UserID);

                if ($gbSend->getResult()):
                    $gbImage = $gbSend->getResult();
                    $gbCreate = ['post_id' => $this->Post, "gallery_image" => $gbImage, "gallery_date" => date('Y-m-d H:i:s')];
                    $insertGb = new Create;
                    $insertGb->ExeCreate("nit_posts_imgs", $gbCreate);
                    $u++;
                endif;
            ...
            if ($u > 1):
                $this->Error = ["Galeria Atualizada: Foram enviadas {$u} imagens este álbum!", WS_ACCEPT];
                $this->Result = true;
            endif;
        endif;
    }
    ...
    public function gbRemove($GbImageId) {
        ...
        $readGb = new Read;
        $readGb->ExeRead("nit_posts_imgs", "WHERE gallery_id = :gb", "gb={$this->Post}");
        if ($readGb->getResult()):
            ...
            $Deleta = new Delete;
            $Deleta->ExeDelete("nit_posts_imgs", "WHERE gallery_id = :id", "id={$this->Post}");
            if ($Deleta->getResult()):
    ...
    private function setData() {
        $Cover = $this->Data['post_capa'];
        $Content = $this->Data['post_desc'];
        unset($this->Data['post_capa'], $this->Data['post_desc']);

        $this->Data = array_map('strip_tags', $this->Data);
        $this->Data = array_map('trim', $this->Data);

        $this->Data['post_nome'] = str_replace('&', 'e', $this->Data['post_nome']);
        $this->Data['post_url'] = Check::Name($this->Data['post_nome']);
        $this->Data['post_data'] = Check::Data($this->Data['post_data']);
        $this->Data['post_capa'] = $Cover;
        $this->Data['post_desc'] = $Content;
    }
    ...
    private function setName() {
        ...
        $readName = new Read;
        $readName->ExeRead(self::Entity, "WHERE {$Where} post_nome = :t", "t={$this->Data['post_nome']}");
        if ($readName->getResult()):
            $this->Data['post_url'] = $this->Data['post_url'] . '-' . $readName->getRowCount();
        endif;
    }
    ...
    private function Create() {
        ...
        if ($cadastra->getResult()):
            $this->Error = ["O álbum {$this->Data['post_nome']} foi cadastrado com sucesso!", WS_ACCEPT];
    ...
    private function Update() {
        ...
        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE post_id = :id", "id={$this->Post}");
        if ($Update->getResult()):
            $this->Error = ["O álbum <b>{$this->Data['post_nome']}</b> foi atualizado com sucesso!", WS_ACCEPT];
            $this->Result = true;
        endif;
    }
