...
/**
 * AdminParceiros.class [ MODEL ADMIN ]
...
class AdminParceiros {
    ...
    public function ExeCreate(array $Data) {
        ...
        $this->UserID = (int) $Data['user_id'];
            ...
            if (isset($upload) && $upload->getResult()):
                ...
                $_SESSION['errCapa'] = "<br>Erro ao enviar Logo:</b> Tipo de arquivo inválido, envie imagens JPG ou PNG!";
                $this->Create();
            endif;
    }
    ...
    public function ExeDelete($SliderId, $UserID)
    {
        ...
        if (!$ReadSlider->getResult()):
            $this->Error = ["O Logo que você tentou deletar não existe!", WS_ERROR];
            $this->Result = false;
        else:
    ...
    private function Create() {
        ...
        if ($cadastra->getResult()):
            $this->Error = ["O logo {$this->Data['slide_title2']} foi cadastrado com sucesso!", WS_ACCEPT];
    ...
    private function Update() {
        ...
        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE slider_id = :id", "id={$this->Slider}");
        if ($Update->getResult()):
            $this->Error = ["O logo <b>{$this->Data['slide_title2']}</b> foi atualizado com sucesso!", WS_ACCEPT];
            $this->Result = true;
        endif;
    }
