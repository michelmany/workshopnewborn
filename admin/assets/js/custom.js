 ...
 *	FotoSites
 *
 *	Developed by: www.nitdesign.com.br
 **/
			...
			$(".datepicker").each(function(i, el)
			{
				var $this = $(el),
					opts = {
					    ...
					    language: attrDefault($this, 'language', 'pt-BR'),
...
jQuery(document).ready(function($)
{
	...
	$("form#login").validate({
		...
		messages: {
			user: {
				required: 'Por favor digite seu email de acesso.'
			},
			pass: {
...
// Adiciona custom Scrollbar e selectboxIt na Categoria Select
jQuery(document).ready(function($)
{
	$("#categoriaSelect").selectBoxIt().on('open', function()
	{
...
$(function() {
    $('#sortable').sortable({
        ...
        update: function(event, ui) {
            ...
            $.ajax({
                url: 'painel.php?exe=slider/set_order',
                type: 'POST',
...
$(document).ready(function(){
	$("#album-image").fadeIn('slow');
});


// Reordena os ítens DOS ALBUNS
$(function() {
    $('#sortableAlbum').sortable({
        ...
        update: function(event, ui) {
    		...
    		//change order in the database using Ajax
            $.ajax({
                url: 'painel.php?exe=albuns/set_order',
                type: 'POST',
                data: {list_order:list_sortable},
                success: function(data) {
...
// Reordena as imagens dos Posts
$(function() {
    $('#sortablePost').sortable({
    	connectWith: ".column2",
    	placeholder: "portlet-placeholder2",
        update: function(event, ui) {
            var list_sortable = $(this).sortable('toArray').toString();
    		...
    		//change order in the database using Ajax
            $.ajax({
                url: 'painel.php?exe=posts/set_order',
                type: 'POST',
                data: {list_order:list_sortable},
                success: function(data) {
...
/* Carrega o Modal de visualização e edição da imagem clicada */

function editModal(imagemId) {

	$.ajax({
	    url: "../admin/system/albuns/modalrequest.php",
	    data: {imagemid:imagemId},
	    type: 'POST',
	    dataType: 'json',
	    success: function(data) {
	    	//console.log(data);
			$("#imgtitle").val(data[0].gallery_title);
			$('#imgid').val(data[0].gallery_id);
			$('#imgedit').attr('src', '../uploads/' + data[0].gallery_image);
			$('.j_modal_show_img').modal('show', {backdrop: 'static'});
	    }
	});
};

	
//Salva o título alterado no Banco.
$('.j_btn_salva').submit(function(){
	...
	$.ajax({
		url: '../admin/system/albuns/ajaxupdate.php',
		data: dados,
		type: 'POST',
		dataType: 'json',
		success: function(data) {
			...
			toastr.success("Título atualizado com sucesso!", "Olá!", opts); 

		}

	});
	return false; // Serve para não dar reload na página!
});

//Salva as configs do admin no banco (nit_site_config).
$('.j_btn_salva_Configs').submit(function(){

	var dados = $(this).serialize();

	$.ajax({
		url: '../admin/system/config/save-configs-ajax.php',
		data: dados,
		type: 'POST',
		dataType: 'json',
		success: function(data) {
			...
			toastr.success("Dados atualizados com sucesso!", "Olá!", opts); 
...
//Salva as configs do admin no banco (nit_site_config).
$('.j_btn_salva_account').submit(function(){

	var dados = $(this).serialize();

	$.ajax({
		url: '../admin/system/account/save-account-ajax.php',
		data: dados,
		type: 'POST',
		dataType: 'json',
		success: function(data) {
...
$('.j_btn_detalhes').click(function(){
	$('.j_barraDetalhes').slideToggle("fadeIn");
});