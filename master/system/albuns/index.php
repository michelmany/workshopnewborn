...
<div class="page-title">
	...
	<div class="breadcrumb-env">
		<ol class="breadcrumb bc-1" >
			...
			<li class="active"><a href="painel.php?exe=albuns/index">Álbuns</a></li>
		</ol>
	</div>
...
<section class="gallery-env">
	<div class="row">
		...
		<div class="col-sm-3 gallery-left">
			<?php 
			    ...
			    if ($empty):
			        WSErro("Oppsss: Álbum não existe!", WS_ALERT);
			    endif;
			 ?>
			<div class="gallery-sidebar">


				<a href="painel.php?exe=albuns/create" class="btn btn-block btn-turquoise btn-icon btn-lg">
		        ...
		        if(isset($delalbumid)):
		     	...
		     	?>
					...
					$readCats = new Read;
					$readCats->ExeRead('nit_albuns_cats', "WHERE user_id = '$user_id' ORDER BY category_title ASC");
					?>		     						
					...
					<ul id="nomesAlbuns" class="list-unstyled">
						...
						<?php foreach($readCats->getResult() as $cats): ?>
							...
							$read->ExeRead("nit_albuns", "WHERE user_id = :userid AND album_categoria_id = :catid ORDER BY album_data DESC", "catid={$category_id}&userid={$user_id}");
							if (!$read->getResult()):
								#Se não encontrar resultados no banco mostra mensagem. Se encontrar, mostra a tabela.
								echo "<small>Nenhum álbum nesta categoria</small>";
							else: ?>							
								...
								foreach($read->getResult() as $row): ?>
									...
									<li class="<?php echo $album_id == $albumid ? "active" : ""; ?>">
										<a href="painel.php?exe=albuns/index&albumid=<?php echo $album_id; ?>">
											<?php echo $album_id == $albumid ? '<i class="fa-folder-open-o"></i>' : '<i class="fa-folder-o"></i>'; ?>
										 	<?php echo $album_nome; ?>
										</a>
		...
		<?php $read->ExeRead("nit_albuns", "WHERE album_id = '$albumID' AND user_id = '$user_id'"); ?>
		...
		<div class="col-sm-9 gallery-right">
			...
			<div class="album-header">
				<div class="row">
					<div class="col-lg-12">		
						<h2><i class="fa-folder-open-o"></i> <?php echo $album_nome; ?></h2>
						...
						<div class="j_albumOptions album-options">
		 					...
		 					<a href="painel.php?exe=albuns/update&albumid=<?= $albumID ?>" class="btn btn-orange" data-toggle="tooltip" data-placement="left" title="Editar álbum"><i class="fa-edit"></i></a>
		 					<a onclick="confirm_modal_del_album('painel.php?exe=albuns/index&delalbum=<?= $albumID ?>', '<?= $album_nome; ?>')" class="btn btn-danger" data-toggle="tooltip" data-placement="left" title="Excluir álbum"><i class="fa-trash-o"></i></a>
						</div>
					</div>

				</div>

				<?php #Consulta quantidade de fotos desse álbum
	            $read->ExeRead("nit_albuns_imgs", "WHERE album_id = '$albumID'");
				...
				<div class="row">
					<div class="j_barraDetalhes detalhesAlbum">
						<ul class="col-lg-12 list-unstyled text-center">
							...
							<li class="col-lg-4"><i class="linecons-eye"></i> <b>Visualizações:</b> <?php echo $album_views; ?></li>
			...
			<div id="album-image" class="row">
				...
				<ul id="sortableAlbum">
					<?php $read->ExeRead("nit_albuns_imgs", "WHERE album_id='$albumID' ORDER BY gallery_order"); ?>
					<?php if ($read->getResult()): ?>
						<?php foreach ($read->getResult() as $gallery): ?>
							...
							<li id="<?php echo $gallery_id; ?>" class="list-unstyled column2">
								...
								<a href="#" class="img-thumbnail" onclick="editModal(<?= $gallery_id ?>);" >
	...
	<?php 
		...
		header('Location: painel.php?exe=albuns/index&empty=true'); #Se não achar resultado joga de volta pro index. Medida de segurança.
	endif; ?>
	<?php endif; ?>
