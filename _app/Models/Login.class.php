...
class Login {
    ...
    private function setLogin() {
        if (!$this->Email || !$this->Senha || !Check::Email($this->Email)):
            ...
            $this->Result = false;
        elseif (!$this->getUser()):
            $this->Error = ['Atenção! Os dados informados não são compatíveis!', WS_ALERT];
            $this->Result = false;
        elseif ($this->Result['user_level'] < $this->Level):
            $this->Error = ["Oppssss, você não tem permissão para acessar esta área!", WS_ERROR];
            $this->Result = false;
        else:
            $this->Execute();
        endif;
    ...
    private function getUser() {
        ...
        $read = new Read;
        $read->ExeRead("nit_users", "WHERE user_email = :e AND user_password = :p", "e={$this->Email}&p={$this->Senha}");
