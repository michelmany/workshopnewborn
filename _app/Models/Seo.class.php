...
class Seo {
    ...
    private function getSeo() {
        ...
        switch ($this->File):
            ...
            case 'post':
                ...
                #esta validação é pra quando o album estiver inativo, conseguimos acessar pelo administrador.
                $Admin = (isset($_SESSION['userlogin']['user_level']) && $_SESSION['userlogin']['user_level'] == 3 ? true : false);
                $Check = ($Admin ? '' : 'post_status = 1 AND');

                $ReadSeo->ExeRead("nit_posts", "WHERE {$Check} post_url = :link", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    ...
                    $this->Data = [$post_nome . ' | ' . SITENAME, $post_desc, HOME . "/post/{$post_url}", BASE . "/uploads/{$post_capa}"];

                    //Album:: conta views do álbum
                    $ArrUpdate = ['post_views' => $post_views + 1];
                    $Update = new Update();
                    $Update->ExeUpdate("nit_posts", $ArrUpdate, "WHERE post_id = :postid", "postid={$post_id}");
                endif;
                break;    


            //SEO:: LISTA Posts - Blog PAGE
            case 'posts':
                $this->Data = ["Blog | " . SITENAME, SITEDESC, HOME . '/posts/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;                    
            ...
            case 'portfolio':
                ...
                #esta validação é pra quando o album estiver inativo, conseguimos acessar pelo administrador.
                $Admin = (isset($_SESSION['userlogin']['user_level']) && $_SESSION['userlogin']['user_level'] == 3 ? true : false);
                $Check = ($Admin ? '' : 'album_status = 1 AND');

                $ReadSeo->ExeRead("nit_albuns", "WHERE {$Check} album_url = :link", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                    $extract = extract($ReadSeo->getResult()[0]);
                    $this->seoData = $ReadSeo->getResult()[0];
                    $this->Data = [$album_nome . ' | ' . SITENAME, $album_desc, HOME . "/portfolio/{$album_url}", BASE . "/uploads/{$album_capa}"];

                    //Album:: conta views do álbum
                    $ArrUpdate = ['album_views' => $album_views + 1];
                    $Update = new Update();
                    $Update->ExeUpdate("nit_albuns", $ArrUpdate, "WHERE album_id = :albumid", "albumid={$album_id}");
                endif;
                break;     
            ...
            case 'workshop':
                ...
                $ReadSeo->ExeRead("nit_workshops", "WHERE workshop_url = :link", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                    $extract = extract($ReadSeo->getResult()[0]);
                    $this->seoData = $ReadSeo->getResult()[0];
                    $this->Data = [$workshop_nome . ' | ' . SITENAME, $workshop_msg, HOME . "/workshop/{$workshop_url}", BASE . "/uploads/{$workshop_capa}"];

                    //Album:: conta views do álbum
                    // $ArrUpdate = ['workshop_views' => $workshop_views + 1];
                    // $Update = new Update();
            ...
            //SEO:: CONTATO
            case 'contato':
                $this->Data = ["Contato | " . SITENAME, SITEDESC, HOME . '/contato/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;  

            //SEO:: CONTRATE
            case 'contrate':
                $this->Data = ["Contrate agora seu site | " . SITENAME, SITEDESC, HOME . '/contrate/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;

            //SEO:: TERMOS
            case 'termos':
                $this->Data = ["Termos de uso | " . SITENAME, SITEDESC, HOME . '/termos/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;

            //SEO:: POLITICA
            case 'politica':
                $this->Data = ["Contrate agora seu site | " . SITENAME, SITEDESC, HOME . '/politica/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;                                                    

            //SEO:: CADASTRA EMPRESA
            case 'sobre':
                $this->Data = ["Sobre | " . SITENAME, SITEDESC, HOME . '/sobre/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;                      
    ...
    private function setTags() {
        ...
        $this->seoTags .= '<meta name="keywords" content="' . KEYWORDS . '"/>' . "\n";
