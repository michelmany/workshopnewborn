...
class Read extends Conn {
     ...
     * @param STRING $ParseString = link={$link}&link2={$link2}
     */
    public function ExeReadInnerJoin($Tabela1, $Tabela2, $Coluna_ref, $Termos = null, $ParseString = null) {
        if (!empty($ParseString)):
            parse_str($ParseString, $this->Places);
        endif;

        $this->Select = "SELECT * FROM {$Tabela1} INNER JOIN {$Tabela2} ON {$Tabela1}.{$Coluna_ref} = {$Tabela2}.{$Coluna_ref} {$Termos}";
        $this->Execute();
    }    
